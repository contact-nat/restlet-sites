...
In order to use the data browser, the Entity Store must be deployed first (see [Entity Store deployment](/documentation/cloud/guide/store/entity-stores/deploy "Entity Store deployment")).
...
- If the *id* is generated by Restlet Cloud, you do not need an *id* column. If an *id* is imported, it will be updated following the existing ids format.
...
Restlet Cloud lets you backup the data stored in an Entity Store for safekeeping and restoration.
