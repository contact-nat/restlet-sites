...
![Install Junit plugin](./images/jenkins_install_junit.png)
...
Now that all the pieces are ready, let's create the job! Click on `New Item`

![New item](./images/jenkins_new_item.png)

Then choose a name for your job and select the type __Pipeline__:

![Create job](./images/jenkins_create_job.png)

You can now save the job. It is ready to run! Just click on `Build now` to run it.

![Build now](./images/jenkins_build_now.png)

The job will run and the Junit Attachments plugin we installed at the begining will parse the reports generated by
